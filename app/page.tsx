...
function EventCalendar({ TH, tasks, sched, onAddEvent, vy, vm, setVY, setVM }: any) {
...
export default function Dashboard() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: TH.bg, color: TH.text, fontFamily: "serif" }}>
      ...
      {/* MODALS (Settings Only for now) */}
      {settingsOpen && <SettingsPanel open={settingsOpen} onClose={() => setSettings(false)} userName={userName} setUserName={setUserName} TH={TH} user={user} t={DICT.ja} />}
      ...
      <nav className="mob-bar" style={{ position: "fixed", bottom: 0, left: 0, right: 0, display: isMobile ? "flex" : "none", background: TH.bg2, borderTop: `1px solid ${TH.border}`, justifyContent: "space-around", padding: 10, zIndex: 1000 }}>
        {["schedule", "partner", "tasks", "chart"].map(k => (
